...
class MissingAPIKeyError(Exception):
    """Raised when the API key is required but not provided."""
    pass
...
class OrchestratorClient:
    ...
    def _validate_api_key(self) -> None:
        """Validate that an API key is configured before making authenticated requests."""
        if not self.api_key:
            raise MissingAPIKeyError(
                "No API key provided. Set AO_API_KEY environment variable or "
                "pass api_key when initializing OrchestratorClient."
            )
